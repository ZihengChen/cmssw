...
double HGCalImagingAlgo::calculateLocalDensity(std::vector<KDNode> &nd,
  ...
  for (unsigned int i = 0; i < nd.size(); ++i) {
    ...
    // if(found_size == 0)
    //   std::cout << "found_size=0 for " << nd[i].dims[0] << " " << nd[i].dims[1] << std::endl;
...
double HGCalImagingAlgo::calculateLocalDensity_BinCPU(Histo2D hist, LayerRecHitsGPU &hits, const unsigned int layer) const {
  double maxdensity = 0.0;
  float delta_c; // maximum search distance (critical distance) for local
                 // density calculation
...
double HGCalImagingAlgo::calculateDistanceToHigher_BinCPU(Histo2D hist, LayerRecHitsGPU &hits, const unsigned int layer) const {
...
int HGCalImagingAlgo::findAndAssignClusters_BinCPU( LayerRecHitsGPU &hits, const unsigned int layer ) const {
  ...
  // buffer for index of hits with clusterIndex but have not pass clusterIndex to their followers
  ...
  for(unsigned int i = 0; i < hits.size(); i++) {
    ...
    // decide a seed
    if (hits[i].delta > delta_c && hits[i].rho >= rho_c) {
      // hits[i] is a seed 
    ...
    // decide a outlier
    if (hits[i].delta > outlierDeltaFactor_*delta_c && hits[i].rho < rho_c) {
      // hits[i] is a outlier
  ...
  for(unsigned i=0; i<hits.size(); ++i)
    std::cout << "RecHit N: " << i << " | clusterIndex: " << hits[i].clusterIndex << " | Delta: " << hits[i].delta << " | NearestHigher: " << hits[i].nearestHigher << " | Density: " << hits[i].rho << std::endl;
